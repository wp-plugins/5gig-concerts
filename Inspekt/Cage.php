 ...
 */
require_once URL_ROOT . 'class/Inspekt.php';
...
//					echo "<pre> Filter $this_filter/$key: "; echo var_dump($this->_source); echo "</pre>\n";
	...
	function slash($key)
	{
		...
		return Inspekt::slash($this->_getValue($key));
	}
	...
	function getHtml($key)
	{
		...
		return Inspekt::getHtml($this->_getValue($key));
	}
     ...
     * @tag filter
     */
	function noTags($key, $allowable_keys='')
	{
		if (!$this->keyExists($key)) {
			return false;
		}
		return Inspekt::noTags($this->_getValue($key), $allowable_keys);
	}
